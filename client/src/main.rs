...
async fn main() -> AnyResult<()> {
    ...
    let watcher_main_task: JoinHandle<AnyResult<()>> = watcher::spawn_watcher(fs_event_tx).await;
    let _dispatcher_main_task: JoinHandle<AnyResult<()>> =
        dispatcher::spawn_dispatcher(fs_event_rx, network_tx).await;
    let _messenger_main_task: JoinHandle<AnyResult<()>> =

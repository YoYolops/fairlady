...
async fn main() -> AnyResult<()> {
    ...
    let messenger_main_task: JoinHandle<AnyResult<()>> =
        messenger::spawn_messenger(messenger_tx).await;
    ...
    match messenger_main_task.await {
        ...
        Err(e) => {
            println!("{:?}", e);
            println!(
                "CUMULUS Server gracefully exiting on messenger error. Without messenger, we cannot work!"
            );
        }
    };
    Ok(())

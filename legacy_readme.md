This was the previous cumulus goals/specifications and dreams.
Before we had discovered Syncthing and how it already does everything we wished doing...

# How to run:
(Promess: i will dockerize this)

Go to file `/cumulus/core/src/constants` and alter this constant:

```rust
pub const OBSERVED_FOLDER_PATH_STRING: &str = "/home/yo/Documents/cumulus/observed";
```

to point to the folder you wanna watch. Now, ensure you have:

1 - Rust 1.90 installed;
2 - Debian installed (12 or 13 are better suited, since i didn't test in other OS...)

After that, inside cumulus' project folder, you can cargo run client and server

# Similar projects:
[Nextcloud Files](https://google.com): Solid and mature. Written in `php`. Centralized. Focused on the end user (companies)

[Syncthing](https://syncthing.net/): `Go`, Descentralized, secure
...
2. We created the foundation for NimbusProtocol, our inter application (client/server) protocol, with simplicity (a lot of it :) in mind.
